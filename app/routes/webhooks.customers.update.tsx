...
/**
 * Webhook handler for customer update events
 * 
 * This webhook is triggered when a customer is updated in Shopify.
 * It syncs the updated customer data to xwanai.com platform.
 * 
...
export const action = async ({ request }: ActionFunctionArgs) => {
  try {
    ...
    // Sync updated customer to xwanai.com
    const result = await syncCustomerToXwanAI(shopifyCustomer, shop);

    if (!result.success) {
      console.error(`Failed to sync customer update ${shopifyCustomer.email} to xwanai.com:`, result.error);
